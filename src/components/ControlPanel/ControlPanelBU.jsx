  ...
  return (
    <div
      className={`fixed bottom-0 left-0 w-full ${
        isClickedCheck ? `bg-${isCorrectAns ? "green" : "red"}-100` : "bg-white"
    ...
    >
      ...
      {!isClickedCheck && (
        <button
          ...
          disabled={!isSelected}
          className={`bg-${
            !isSelected && questionType != QUESTION_TYPES.LEARN
              ? "gray-400"
              : "orange-500"
          } text-white rounded-lg px-14 py-2  opacity-${
            !isSelected && questionType != QUESTION_TYPES.LEARN ? "70" : "100"
          } cursor-${
            !isSelected && questionType != QUESTION_TYPES.LEARN
              ? "not-allowed"
              : "pointer"
