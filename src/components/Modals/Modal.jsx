...
const Modal = ({ children, onClose, isShowCloseButton = false }) => {
  ...
  return ReactDOM.createPortal(
    <div
      className="fixed inset-0 flex justify-center items-center p-4 z-50 bg-black bg-opacity-50" // Added z-50 and dark background
      onClick={onClose}
    >
      <div
      ...
      >
        {isShowCloseButton && (
          <button
            onClick={() => closeHandler()}
            className="absolute top-3 right-3 text-gray-600 hover:text-gray-1000 hover:shadow-md hover:border hover:rounded-xl z-10"
            aria-label="Close"
          >
            <IoMdClose className="text-2xl" />
          </button>
