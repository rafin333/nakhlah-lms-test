...
import { useGetContentDetailsByClausesQuery } from "@/redux/features/contentDetails/contentDetailsApi";
...
const SentenceMaking = ({
  ...
  const { data: clauseContentDetailsData } = useGetContentDetailsByClausesQuery(
    { ...query }
  );
  console.log(questionDetails?.content?.id);
  console.log(questionDetails?.mediaTypes);
  console.log(questionDetails);
  console.log(clauseContentDetailsData);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log(questionDetails?.content?.id, contentDetailsByClauses);
  ...
  const handleClick = (clauseId, clauseTitle, sequence) => {
    ...
    // if (questionDetails?.contentMediaTypes?.audio && audio?.attributes?.audio) {
    //   TextToSpeech(audio?.attributes?.audio, questionDetails?.language);
    // }
    setIsSelected(true);
  ...
  useEffect(() => {
    const fetch = async () => {
      if (isClickedCheck && selectedClauses.length > 0) {
        let clauseData = clauseContentDetailsData.filter(
          (clauseContentDetail) =>
            clauseContentDetail?.attributes?.content?.data?.id ==
            questionDetails?.content?.id
        );
        clauseData = clauseData.sort(
          (a, b) => a.attributes?.sequence - b.attributes?.sequence
        );
        let contentWithSequence = clauseData
          .map((clause) => clause?.attributes?.title)
          .join(" ");
        const selectedClauseSentence = selectedClauses
          .map((selectedClause) => selectedClause?.title)
          .join(" ");
        let isRightAns =
          contentWithSequence.toLowerCase() ==
          selectedClauseSentence.toLowerCase();
