...
const AuthForm = ({ onLoginSuccess }) => {
  ...
  useEffect(() => {
    // Only clear info if we are on the main login page, not inside a modal
    if (!onLoginSuccess) {
      removeUserInfo();
    }
  }, [onLoginSuccess]);
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      if (typeof comesFrom !== 'undefined' && comesFrom === "LearnerInfo") {
  ...
  return (
    <GoogleOAuthProvider clientId="849081741540-kpbhg2m2v4nocop4un022gchsogfsddl.apps.googleusercontent.com">
      <div className={`flex items-center justify-center bg-[rgb(248,241,219)] ${onLoginSuccess ? 'p-0' : 'min-h-screen px-4 py-12'}`}>
