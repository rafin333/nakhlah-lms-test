...
const LevelButton = ({
  ...
  const [size, setSize] = useState(() => {
    ...
    return { width: 15 * vh, height: 15 * vh };
  });
  useEffect(() => {
    ...
    <div className={styles.levelButton}>
      ...
      {level?.mysteryBox ? (
        isUnlocked ? (
          ...
          <div className={styles.mysteryLockButtonInner}></div> // 🔒 locked mystery
        )
      ) : (
        // ✅ Normal Level Case
        isUnlocked ? (
          ...
          <div className={styles.lockButtonInner}></div> // 🔒 locked normal level
        )
      )}
    </div>
  );
